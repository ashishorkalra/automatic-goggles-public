    ...
    try:
        ...
        try:
            ...
            if "include_reasoning" in params:
                ...
                print(
                    f"✅ include_reasoning parameter found with default: {default_value}"
                )
        ...
        try:
            from transtype.processor import (
                FieldExtractionSignature,
                FieldExtractionSignatureNoReasoning,
            )

            print(
                "✅ Both FieldExtractionSignature and FieldExtractionSignatureNoReasoning exist"
            )
            ...
            no_reasoning_outputs = getattr(
                FieldExtractionSignatureNoReasoning, "__annotations__", {}
            )

            print(
                f"✅ FieldExtractionSignature fields: {list(reasoning_outputs.keys())}"
            )
            print(
                f"✅ FieldExtractionSignatureNoReasoning fields: {list(no_reasoning_outputs.keys())}"
            )

            # Check that reasoning signature has more fields (includes reasoning)
            if len(reasoning_outputs) > len(no_reasoning_outputs):
                print(
                    "✅ FieldExtractionSignature has more fields (includes reasoning)"
                )
            else:
                print(
                    "⚠️  FieldExtractionSignature should have more fields than NoReasoning version"
                )
        ...
        print("\n" + "=" * 50)
        print(
            "🎉 All tests passed! Optional reasoning functionality is working correctly."
        )
        return True
...
def test_model_serialization():
    ...
    try:
        ...
        if (
            dict_with["field_reason"] == "Some reasoning"
            and dict_without["field_reason"] is None
        ):
